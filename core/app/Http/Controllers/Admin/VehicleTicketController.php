...
class VehicleTicketController extends Controller
{
    ...
    public function ticketPriceStore(Request $request){
        ...
        if($check){
            $notify[] = ['error', "Il n'est pas possible d'autoriser la duplication du type de bus et de l'itinéraire"];
        ...
        $notify[] = ['success', 'Le prix du ticket a été ajouté avec succès'];
        return back()->withNotify($notify);
    }
    ...
    public function ticketPriceUpdate(Request $request, $id){
        ...
        $notify = ['success' => true, 'message'=>'Prix mis à jour avec succès'];
    ...
    public function checkTicketPrice(Request $request){
        ...
        if(!$check){
            return response()->json(['error' => 'Prix du ticket ajouté pour cette combinaison voyage-route. Veuillez ajouter le prix du ticket avant de créer un voyage.']);
        }
    }
}
