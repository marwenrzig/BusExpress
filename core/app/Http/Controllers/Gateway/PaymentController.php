...
class PaymentController extends Controller
{
    ...
    public function depositInsert(Request $request)
    {
        ...
        if (!$gate) {
            $notify[] = ['error', 'Moyen de paiement non valide'];
            return back()->withNotify($notify);
        }

        if ($gate->min_amount > $bookedTicket->sub_total || $gate->max_amount < $bookedTicket->sub_total) {
            $notify[] = ['error', 'S’il vous plaît suivre la limite de paiement'];
            return back()->withNotify($notify);
        }
    ...
    public function depositConfirm()
    {
        ...
        if ($deposit->method_code >= 1000) {
            ...
            $notify[] = ['success', 'Votre demande de paiement est mise en file d’attente pour approbation.'];
            return back()->withNotify($notify);
        }
    ...
    public static function userDataUpdate($trx)
    {
        ...
        if ($data->status == 0) {
            ...
            $adminNotification->titre = 'Paiement réussi via '.$data->gatewayCurrency()->name;
            ...
            notify($user, 'PAYMENT_COMPLETE', [
                ...
                'source' => $bookedTicket->pickup->nom,
                'destination' => $bookedTicket->drop->nom
            ]);
    ...
    public function manualDepositUpdate(Request $request)
    {
                        ...
                        if ($inVal->type == 'file') {
                            if ($request->hasFile($inKey)) {
                                try {
                                    ...
                                    $notify[] = ['error', 'Impossible de télécharger votre fichier' . $inKey];
        ...
        $adminNotification->titre = 'Payment request from '.$data->user->username;
        ...
        $notify[] = ['success', 'Votre demande de paiement a été prise en compte.'];
