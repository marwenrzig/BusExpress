                            ...
                            @forelse($trips as $item)
                                <tr>
                                    ...
                                    <td data-label="@lang('AC / Non-AC')">
                                        {{ __($item->fleetType->est_climatise == 1 ? 'AC' : 'Non-Ac') }}
                                    </td>
                        ...
                        <div class="form-group">
                            ...
                            <select name="fleet_type" class="select2-basic fleet_type1" required>
                                ...
                                @foreach ($fleetTypes as $item)
                                    <option value="{{ $item->id }}" data-name="{{$item->nom}}">{{ __($item->nom) }}</option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <select name="schedule" class="select2-basic schedule1" required>
                                ...
                                @foreach ($schedules as $item)
                                    <option value="{{ $item->id }}" data-name="{{ showDateTime($item->depart, 'h:i a').' - '. showDateTime($item->arrive, 'h:i a') }}">{{ __(showDateTime($item->depart, 'h:i a').' - '. showDateTime($item->arrive, 'h:i a')) }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="form-group">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="form-group">
                            ...
                            <select name="fleet_type" class="select2-basic fleet_type2" required>
                                <option value="">@lang('Select an option')</option>
                                @foreach ($fleetTypes as $item)
                                    <option value="{{ $item->id }}" data-name="{{$item->nom}}">{{ ($item->nom) }}</option>
                                @endforeach
                            </select>
                        </div>
    ...
    <script>
        (function ($) {
            ...
            $('.editBtn').on('click', function () {
                ...
                modal.find('select[name=depart]').data('name',trip.start_from.name);
                modal.find('select[name=arrive]').data('name',trip.end_to.name);
                ...
                modal.find('select[name=depart]').val(trip.start_from);
                modal.find('select[name=arrive]').val(trip.end_to);
            ...
            $(document).on('change', '.fleet_type1, .route1', function () {
                makeTitle('addModal');
            });

            $(document).on('change', '.fleet_type2, .route2', function () {
                makeTitle('editModal');
            });
            ...
            function makeTitle(modalName){
                ...
                var data2 = modal.find('select[name="route"]').find("option:selected").data('name');

                if (data1 != undefined && data2 != undefined) {
                    var title = title + data1 + "-" + data2;
                }
